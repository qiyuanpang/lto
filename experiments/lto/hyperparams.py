...
def gen_fcns(interval1, interval2, max_dim_1, max_dim_2, num_fcns, session, gpu_id = 0, num_inits_per_fcn = 1, num_points_per_class = 50):
    ...
    fcn_objs = []
    dims = np.random.randint(int(max_dim_1/2), max_dim_1, size=num_fcns)
    ...
    if 1:
       for k in range(num_fcns):
           ...
           data = np.eye(max_dim)*max(max(data1))
           ...
           for j in range(num_inits_per_fcn):
              init_locs[:, k*num_inits_per_fcn+j] = CGM(data, labels, np.random.randn([max_dim, 1]), 8)
              fcn_objs.append(fcn)
...
if os.path.isfile(dataset_file):
    ...
    fcns,fcn_family = gen_fcns([0, a], [0, b], max_dim_1, max_dim_2, num_fcns, session, gpu_id)
