...
class Algorithm(object):
    ...
    def iteration(self, sample_lists):
        ...
        print('set_interp_values')
        self._set_interp_values()
        print('update_dynamics')
        ...
        for m in range(self.M):
            ...
            print('policy_kl for ', m)
            self.cur[m].pol_info.init_kl = self._policy_kl(m)[0]
        print('running time for policy_kl:', time.perf_counter()-start)
        ...
        for inner_itr in range(self._hyperparams['inner_iterations']):
            ...
            if self.iteration_count > 0 or inner_itr > 0:
                # Update the policy.
                print('update_policy, iteration_count=', self.iteration_count, 'inner_itr=', inner_itr)
                start = time.perf_counter()
                self._update_policy(inner_itr)
                print('running time for update_policy:', time.perf_counter()-start)
            print('update_policy_fit, iteration_count=', self.iteration_count, 'inner_itr=', inner_itr)
            start = time.perf_counter()
            for m in range(self.M):
                print('   condtion', m)
                self._update_policy_fit(m)  # Update policy priors.
            print('running time for update_policy_fit:', time.perf_counter()-start)
            if self.iteration_count > 0 or inner_itr > 0:
                step = (inner_itr == self._hyperparams['inner_iterations'] - 1)
                # Update dual variables.
                print('policy_dual_step, iteration_count=', self.iteration_count, 'inner_itr=', inner_itr)
                start = time.perf_counter()
                for m in range(self.M):
                    print('dual_step, condtion', m)
                    self._policy_dual_step(m, step=step)
                print('running time for policy_dual_step:', time.perf_counter()-start)
            print('update_trajectories, iteration_count=', self.iteration_count, 'inner_itr=', inner_itr)
            start = time.perf_counter()
            self._update_trajectories()
            print('running time for update_trajectories:', time.perf_counter()-start)
    ...
    def _set_interp_values(self):
        ...
        #### Cannot find 'lg_step_schedule' ?
    def _update_step_size(self):
        """ Evaluate costs on samples, and adjust the step size. """
        # Evaluate cost function for all conditions and samples.
        for m in range(self.M):
            print('in update_step_size, update_policy_fit, cond=', m)
            self._update_policy_fit(m, init=True)
            print('eval_cost, cond=', m)
            self._eval_cost(m)
    ...
    def _update_policy_fit(self, m, init=False):
        ...
        print('  in update_policy_fit, policy_prior.fit')
        start = time.perf_counter()
        ...
        print('  running time for cholesky:', time.perf_counter()-start)
    ...
    def _update_dynamics(self):
        ...
        for m in range(self.M):
            ...
            if self.cur[m].traj_info.dynamics.get_prior():
            ...
            ## When DynamicsLRFixed used, prior should be None.
    ...
    def _eval_cost(self, cond):
        ...
        for n in range(N):
            ...
            cc[n, :] = self.cost[cond].weight * dU * l ## why time dU here?
            cs[n, :] = l
    ...
    def compute_costs(self, m):
        ...
        for t in range(T):
            ...
            #             PKLv[t, :] * wt) / (eta + wt)
            fCm[t, :, :] = Cm[t, :, :] + PKLm[t, :, :] * wt - TKLm[t, :, :] * wt
            fcv[t, :] = cv[t, :] + PKLv[t, :] * wt - TKLv[t, :] * wt
    ...
    def _measure_ent(self, m):
        ...
        ## no need to consider ent_reg (\mu_t)?
        ent = 0
