...
class TfSolver:
    def __init__(self, solver_op, loss, summary_op):
        self.solver_op = solver_op
        self.loss = loss
        self.summary_op = summary_op

    def __call__(self, feed_dict, sess, device_string):
        with tf.device(device_string):
            loss = sess.run([self.loss, self.solver_op, self.summary_op], feed_dict)
            return loss[0]
...
class TfSolver:
    ...
    def __call__(self, feed_dict, sess, device_string, additional_tensors = None):
    ### why use device_string="/cpu:0" ?
        #print('TfSolver:', device_string)
