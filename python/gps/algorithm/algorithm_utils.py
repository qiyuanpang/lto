...
import numpy as np
import scipy.linalg
#from scipy import scipy.linalg
...
class PolicyInfo(BundleType):
    ...
    def traj_distr(self):
        ...
        for t in range(T):
            ## compute (chol_pol_S[t, :, :].T * chol_pol_S[t, :, :])^{-1}
...
def gauss_fit_joint_prior(pts, mu0, Phi, m, n0, dwts, dX, dU, sig_reg, clipping_thresh = None):
    ...
    fd = scipy.linalg.solve(sigma[:dX, :dX], sigma[:dX, dX:dX+dU], assume_a='sym', overwrite_a=True).T
    ori_fd = fd
