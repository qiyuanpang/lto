        ...
        def construct_graph():
            if "graph_def" in self.options:
                ...
                for param_name in self.param_defns:
                    #print param_name
                    #print tensor_names["params"][param_name]
                ...
                if ("disable_hess" not in self.options) or (not self.options["disable_hess"]):
                    ...
                    for i in range(len(self.num_dims)):     # Iterate over each variable group
                        ...
                        #print(rows_of_block_cols)
                        #print(tf.concat([row[0] for row in rows_of_block_cols], axis = 1))
                        block_cols_of_block_cells = [tf.transpose(tf.concat([row[j] for row in rows_of_block_cols], axis = 1), name="%shess_%d_%d" % (prefix,i,j)) for j in range(i+1)]
                        self.hess_.append(block_cols_of_block_cells)
...
class Fcn(object):
    ...
    def evaluate(self, x):
        ...
        #print len(x), self.subsampled_param_vals['data'].shape, self.subsampled_param_vals['labels'].shape
...
class QuadraticNormFcnFamily(FcnFamily):
    def __init__(self, dim, **kwargs):
        def fcn(x, params):
            weights = tf.slice(x[0], [0,0], [dim,-1]) 
            #print(tf.shape(weights))
            pred = tf.subtract(tf.matmul(params['data'], weights), params['labels'])
            loss = tf.nn.l2_loss(pred)/tf.nn.l2_loss(params['labels'])
            #loss = tf.nn.l2_loss(pred)
            #print(tf.nn.l2_loss(params['labels']))
            #print(loss)
            #with tf.Session() as sess:
            #print(tf.shape(params['labels']))
            #print(tf.shape(params['data']))
            #print(loss)
            return loss
...
class LagrangianFcnFamily4Eig(FcnFamily):
    
    def __init__(self, dim, **kwargs):
        def fcn(x, params):
            #print(dim, tf.shape(x[0]))
            phi = tf.slice(x[0], [0, 0], [dim-1, -1])
            lambdas = tf.slice(x[0], [dim-1, 0], [1, -1])
            #pT = tf.transpose(phi)
            App = tf.matmul(params['data'], tf.matmul(phi, phi, transpose_b=True), transpose_a=False)
            pIp = tf.matmul(phi, phi, transpose_a = True) - 0.1
            #pAp = tf.tensordot(phi, tf.matmul(params['data'], phi), 1)
            #pIp = tf.tensordot(phi, phi, 1) - 1
            #print(tf.shape(pAp), tf.shape(pIp), tf.shape(lambdas))
            L = tf.subtract(tf.subtract(tf.linalg.trace(params['data']), tf.linalg.trace(App)), tf.matmul(pIp, lambdas))
            L = L/tf.linalg.trace(params['data'])
            #print('???', tf.shape(L), L[0,0])
            L = L[0, 0]
            return tf.math.exp(L)

        FcnFamily.__init__(self, fcn, [dim], {"data": {"type": tf.float64, "subsampled": False}, "labels": {"type": tf.float64, "subsampled": False}}, {"dim": dim}, **kwargs)

class LagrangianFcn4Eig(Fcn):
...
class LagrangianFcnFamily4Eig1(FcnFamily):
    ...
    def __init__(self, dim, **kwargs):
        def fcn(x, params):
            ...
            lambdas = tf.slice(x[0], [dim-1, 0], [1, -1])
            #pT = tf.transpose(phi)
            A2 = tf.matmul(params['data'], params['data'])
            Acc = tf.matmul(params['data'], tf.matmul(phi, phi, transpose_b=True))
            ccA = tf.matmul(tf.matmul(phi, phi, transpose_b=True), params['data'])
            cccc = tf.matmul(tf.matmul(phi, phi, transpose_b=True), tf.matmul(phi, phi, transpose_b=True))
            cc = tf.matmul(tf.matmul(phi, phi, transpose_a = True) - 1, lambdas)
            #pAp = tf.tensordot(phi, tf.matmul(params['data'], phi), 1)
            #pIp = tf.tensordot(phi, phi, 1) - 1
            #print(tf.shape(pAp), tf.shape(pIp), tf.shape(lambdas))
            #L = tf.subtract(tf.subtract(tf.linalg.trace(params['data']), tf.linalg.trace(App)), tf.matmul(pIp, lambdas))
            #L = L/tf.linalg.trace(params['data'])
            L = tf.linalg.trace(A2+Acc+ccA+cccc)+cc
            #print('???', tf.shape(L), L[0,0])
            L = L[0, 0]
            return tf.math.exp(L)
...
class QuadraticNormCompFcnFamily(FcnFamily):
    def __init__(self, dim, **kwargs):
        def fcn(x, params):
            weights = tf.slice(x[0], [0,0], [dim,-1])
            #print(tf.shape(weights))
        ...
        FcnFamily.__init__(self, fcn, [dim], {"data": {"type": tf.complex64, "subsampled": True}, "labels": {"type": tf.complex64, "subsampled": True}}, {"dim": dim}, **kwargs)

class QuadraticNormCompFcn(Fcn):
...
class TraceNormFcnFamily(FcnFamily):

    def __init__(self, dim, **kwargs):
        def fcn(x, params):
            weights = tf.slice(x[0], [0,0], [dim,-1])
            #y = tf.matmul(tf.transpose(weights), tf.matmul(params['data'], weights))
            #print tf.shape(params['data']), tf.shape(weights), tf.shape(tf.matmul(params['data'], weights))
            y = tf.reduce_sum(tf.multiply(weights, tf.matmul(params['data'], weights)))
            #print(tf.shape(y))
            #xx = tf.matmul(tf.transpose(weights), weights)
            #shape = tf.shape(xx)
            #print(shape[0], type(shape[0]))
            z = tf.subtract(tf.constant(2.0, dtype='float64'), tf.reduce_sum(tf.multiply(weights, weights)))
            loss = tf.reduce_sum(tf.subtract(tf.multiply(z, y), params['eig']))
            #print(loss)
            return loss
        FcnFamily.__init__(self, fcn, [dim], {"data": {"type": tf.float64, "subsampled": True}, "labels": {"type": tf.float64, "subsampled": True}, "eig": {"type": tf.float64, "subsampled": True}}, {"dim": dim}, **kwargs)

class TraceNormFcn(Fcn):
    def __init__(self, family, data, labels, eig, *args, **kwargs):
        Fcn.__init__(self, family, {"data": data, "labels": labels, "eig": eig}, *args, **kwargs)
...
class TraceNorm2FcnFamily(FcnFamily):

    def __init__(self, dim, **kwargs):
        def fcn(x, params):
            weights = tf.slice(x[0], [0,0], [dim,-1])
            #print dim, params['eig']
            #print(tf.shape(weights), tf.shape(tf.transpose(weights)))
            #y = tf.matmul(tf.transpose(weights), tf.matmul(params['data'], weights))
            #print tf.shape(params['data']), tf.shape(weights), tf.shape(tf.matmul(params['data'], weights))
            y = tf.reduce_sum(tf.multiply(weights, tf.matmul(params['data'], weights)))
            #print(tf.shape(y))
            ...
            #print(shape[0], type(shape[0]))
            z = tf.subtract(tf.constant(2.0, dtype='float64'), tf.reduce_sum(tf.multiply(weights, weights)))
            loss = tf.reduce_sum(tf.subtract(tf.multiply(z, y), -2.0))
            #print(loss)
            return loss
        FcnFamily.__init__(self, fcn, [dim], {"data": {"type": tf.float64, "subsampled": True}, "labels": {"type": tf.float64, "subsampled": True}}, {"dim": dim}, **kwargs)

class TraceNorm2Fcn(Fcn):
    def __init__(self, family, data, labels, *args, **kwargs):
        Fcn.__init__(self, family, {"data": data, "labels": labels}, *args, **kwargs)
