...
class AgentLTO(Agent):
    ...
    def sample(self, policy, condition, verbose=False, save=True, noisy=True, usescale=True):
        """
        ...
        for t in range(self.T):
            ...
            if noisy:
                U[t, :] = policy.act(X_t, obs_t, t, noise[t, :], usescale)
            else:
                U[t, :] = policy.act(X_t, obs_t, t, None, usescale)
            if (t+1) < self.T:
                for _ in range(self._hyperparams['substeps']):
                    ### what is this substeps for??
